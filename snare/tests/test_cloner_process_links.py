...
class TestProcessLinks(unittest.TestCase):
    ...
    def test_process_link_scheme(self):
        ...
        async def test(url_param):
            ...
            self.return_content = await self.handler.runner.process_link(url_param, self.level)
            self.qsize = self.handler.runner.new_urls.qsize()
    ...
    def test_process_link_relative(self):
        ...
        async def test():
            if not self.handler.runner:
                raise Exception("Error initializing Cloner!")
            self.return_content = await self.handler.runner.process_link(self.url, self.level)
            self.return_url, self.return_level, self.return_try_count = (await self.handler.runner.new_urls.get()).values()
        ...
        self.handler.runner.moved_root = yarl.URL("http://example2.com")
    ...
    def test_check_host(self):
        ...
        async def test():
            if not self.handler.runner:
                raise Exception("Error initializing Cloner!")
            self.return_content = await self.handler.runner.process_link(self.url, self.level, check_host=True)
            self.qsize = self.handler.runner.new_urls.qsize()
    ...
    def test_process_link_unicode_error(self, url):
        ...
        async def test():
            if not self.handler.runner:
                raise Exception("Error initializing Cloner!")
            self.return_content = await self.handler.runner.process_link(self.root, self.level)
